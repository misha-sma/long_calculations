...
public class BigNumber {
	...
	public static BigNumber getOne() {
		...
		number.digits[PRECISION] = 1;
		number.length = PRECISION + 1;
		return number;
	}

	public static BigNumber getOneFive() {
		BigNumber number = new BigNumber();
		number.digits[PRECISION - 1] = 2;
	...
	private BigNumber getTmp(int length) {
		BigNumber number = new BigNumber();
	...
	public BigNumber divideOne() {
		List<Integer> resultList = new ArrayList<Integer>(PRECISION);
		BigNumber tmp = getTmp(length);
		int megaCounter = length - 1;
		while (true) {
			int compareResult = tmp.compareTo(this);
			if (compareResult < 0) {
				resultList.add(0);
				++megaCounter;
				if (megaCounter >= PRECISION) {
					break;
				}
				System.arraycopy(tmp.digits, 0, tmp.digits, 1, tmp.length);
				if (tmp.length == 0) {
